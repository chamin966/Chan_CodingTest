...
function bf(start) {
  ...
  for (let i = 0; i < n; i++) { // 전체 n번의 라운드(노드의 개수만큼)를 반복
    for (let j = 0; j < m; j++) {// 매 반복마다 "모든 간선"을 확인하며
      // cost는 curNode에서 nextNode로 가는 비용이다.
      let [cur, nextNode, cost] = edges[j];
      // 현재 간선을 거쳐서 다른 노드로 이동하는 거리가 더 짧은 경우
      if (dist[cur] != INF && dist[nextNode] > dist[cur] + cost) {
        // dist[node]는 시작노드에서 node 까지 가는 비용을 의미한다.
        dist[nextNode] = dist[cur] + cost;
        // n번째 라운드에서도 if문 내부로 들어 값이 갱신된다면 음수 순환이 존재
